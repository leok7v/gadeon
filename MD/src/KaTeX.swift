...
//  /System/Library/Fonts/Supplemental and which this package carries in
//  Resources for iOS, where the system font does not exist.
//
...
public final class MathFontFile {
    ...
    // needed: MarkdownPDF.export is a nonisolated async function, so a
    // transcript export lays out and draws formulas off the main thread
    // while the screen draws its own, and a lock on layout alone leaves
    // ctFont racing -- ThreadSanitizer reports it, and it segfaults.
    static let lock = NSLock()
    nonisolated(unsafe) private static var cached: MathFontFile?

    // Two places, in this order: this package's own bundle, which is
    // where iOS finds its copy, then the system font macOS ships. No
    ...
    public static func shared() throws -> MathFontFile {
        var candidates: [URL] = []
        if let u = Bundle.module.url(forResource: "STIXTwoMath",
                                     withExtension: "otf") {
            candidates.append(u)
        }
