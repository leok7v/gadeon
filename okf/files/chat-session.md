...
`MediaEncoder`, and the modalities it offers are the encoder's. It holds no
view state. After a build it reads the model file once so the first prompt
faults no weights in from disk, when the file fits in memory.
