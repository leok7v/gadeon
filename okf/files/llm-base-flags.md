...
one `Knob` in `Flags.registry`: its kebab-case argv name, whether it takes a
value, a help string, a scope, and a default. `Flags.value` / `.int` /
`.double` / `.uint64` / `.on` resolve argv, then a `Flags.store`d
`UserDefaults` write, then the registry default -- no environment fallback.
