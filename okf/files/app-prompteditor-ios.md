...
`FocusRequest` and goes out as the `editing` report, one writer each; a
paste at or over `ChatModel.pasteAttachBytes` is refused by the
should-change delegate and becomes a Text attachment instead.
