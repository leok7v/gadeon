...
sits in the turn and only the expansion lives here, which is the same split
HF draws between jinja and the processor. A span may also carry the patch
grid its rows came from, which the LM ropes it at, and the begin/end pair
the template wrote around its placeholder when the block brings its own,
