...
// A dense row read as 32-wide blocks, so the same tile serves the
// unquantized matrices; the two 16-halves are `il` 0 and 1 like Q8_0's.
...
// argmax_rows: one threadgroup per row of a [rows, n] logit block, writing
...
// threadgroup per token-row. Rows sit xStride / yStride floats apart, so a
// caller can read one half of a wide row or write into one.
